...
func _initialize() -> void:
	var good:=FarmSettings.normalized({"volume":9,"sensitivity":-1,"quality":2,"fps":120,"fullscreen":false})
	assert(good.volume==1 and good.sensitivity==.25 and good.quality==2 and not good.fullscreen and good.fps==120)
	var bad:=FarmSettings.normalized({"volume":NAN,"sensitivity":"bad","quality":99,"fps":-1,"fullscreen":"false"})
