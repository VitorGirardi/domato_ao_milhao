...
func _initialize() -> void:
	...
	assert(restored.items[0].kind=="stable" and restored.serialize().version==18)
	var old:=state.serialize();old.version=16;old.items=[]
	assert(restored.restore(old) and restored.items.is_empty())
	var bad:=old.duplicate(true);bad.version=19;var before:=restored.serialize()
	assert(not restored.restore(bad) and restored.serialize()==before)
	print("STABLE_STATE_OK: level gate, finite cost, rotated entrance, move, save18 and legacy16")
	quit()
