...
try:
    for mode in ['host','client']:
        ...
        proc=subprocess.Popen([a.godot,*display,'--audio-driver','Dummy','--path',str(root),'--script','res://tests/test_network.gd','--',mode,str(flags)],env=env,stdout=log,stderr=subprocess.STDOUT)
    ...
    for proc,log,mode in processes:
        ...
        assert proc.returncode==0 and 'NETWORK_QA_OK' in output and 'ERROR' not in output, (mode,flags)
