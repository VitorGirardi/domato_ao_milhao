...
`tools/build_audio.py` gera todos os 24 arquivos com Python e NumPy, usando semente fixa. Os WAVs e o manifesto com duração, pico e RMS ficam em `assets/audio`. Os timbres e chamados dos animais são estilizados e sintetizados, não gravações de animais reais.

## No jogo

- Música contínua no menu e na fazenda, mais baixa durante a partida/pausa e atenuada quando a janela perde foco.
- Vento leve, água conforme a proximidade do rio e chamados ocasionais dos animais e pássaros existentes por perto.
...
- Vozes limitadas: quatro efeitos e quatro fontes espaciais. Sons não criam nós indefinidamente.
...
`tests/test_audio.gd` exige APPDATA isolado contendo test-results. Valida os 24 clipes, a duração/loop, buses, silêncio por categoria, passos/parada/menu, atenuação do rio, limite de vozes e ausência de mutações da fazenda. Executar com --headless --audio-driver Dummy. O gerador valida forma de onda finita e margem de pico; efeitos terminam em zero e a música possui borda contínua.
